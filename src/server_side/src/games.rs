...
pub fn game(mut stream: &mut TcpStream) -> Result<(), Box<dyn std::error::Error>> {
    ...
    loop {
        ...
        world.move_object(
            player_id,
            5 * (input.key_d as i16 - input.key_a as i16),
            f32toi16(player_yvel),
        );
        //jump
        ...
        if player_touching {
            if input.key_w && player_yvel == 0_f32 {
                player_yvel -= 10_f32;
            }
            player_yvel = 0_f32;
        } else {
...
fn resolve_platforms_collision(world: &mut GameWorld, operation: u32, ids: &Vec<u32>) -> bool {
    ...
    for plat in ids {
        ...
        if world.resolve_collision(operation, *plat) {
            collided = true;
        }
    }
    collided
...
fn loadlevel(
    ...
    player_id: u32,
    win_rect_id: u32,
    platform_ids: &mut Vec<u32>,
    mut platforms: Vec<u32>,
    start: (i16, i16),
    end: (i16, i16),
) {
    platform_ids.clear();
